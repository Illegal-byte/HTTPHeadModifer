...
{
    public static byte[] addIPHead(final IExtensionHelpers helpers, final IHttpRequestResponse requestResponse, final String key) {
        ...
        map.put("AllIP", "X-Forwarded-For: 127.0.0.1\r\nX-Forwarded-Host: 127.0.0.1\r\nX-remote-IP: 127.0.0.1\r\nX-Originating-IP: 127.0.0.1\r\nX-remote-addr: 127.0.0.1\r\nTrue-Client-IP: 127.0.0.1\r\nClient-IP: 127.0.0.1\r\nX-Client-IP: 127.0.0.1\r\nX-Real-IP: 127.0.0.1");
        final byte[] request = requestResponse.getRequest();
        final IRequestInfo requestInfo = helpers.analyzeRequest(request);
        final int bodyOffset = requestInfo.getBodyOffset();
        final String body = new String(request, bodyOffset, request.length - bodyOffset);
        Boolean isExists = false;
        final List<String> headers = helpers.analyzeRequest(request).getHeaders();
    ...
    public static byte[] modifyUserAgent(final IExtensionHelpers helpers, final IHttpRequestResponse requestResponse, final String uaid) {
        final Map<String, String> map = new HashMap<String, String>();
        ...
        for (int i = 0; i < headers.size(); ++i) {
            if (headers.get(i).startsWith("User-Agent")) {
                final String strUA = String.format("User-Agent: %s", map.get(uaid));
                headers.set(i, strUA);
                isExists = true;
            }
        }
    ...
    public static byte[] addOrigin(final IExtensionHelpers helpers, final IHttpRequestResponse requestResponse) {
        final IHttpService httpService = requestResponse.getHttpService();
        String url;
        ...
        for (int i = 0; i < headers.size(); ++i) {
            if (headers.get(i).startsWith("Origin:")) {
                final String strOrigin = String.format("Origin: %s", url);
    ...
    public static byte[] updateCookie(final IBurpExtenderCallbacks callbacks, final IExtensionHelpers helpers, final IHttpRequestResponse requestResponse) {
        final byte[] request = requestResponse.getRequest();
        final PrintWriter stdout = new PrintWriter(callbacks.getStdout(), true);
        final String shorturl = requestResponse.getHttpService().toString();
        stdout.println("[+] " + shorturl);
        final String latestCookie = getLatestCookieFromHistory(callbacks, helpers, shorturl);
    ...
    public static IHttpRequestResponse[] Reverse(final IHttpRequestResponse[] input) {
        for (int start = 0, end = input.length - 1; start < end; ++start, --end) {
            final IHttpRequestResponse temp = input[end];
    ...
    public static String getLatestCookieFromHistory(final IBurpExtenderCallbacks callbacks, final IExtensionHelpers helpers, final String shortUrl) {
        final IHttpRequestResponse[] historyMessages = Reverse(callbacks.getProxyHistory());
        String lastestCookie = null;
        IHttpRequestResponse[] array;
        for (int length = (array = historyMessages).length, i = 0; i < length; ++i) {
            final IHttpRequestResponse historyMessage = array[i];
            final IRequestInfo hisAnalyzedRequest = helpers.analyzeRequest(historyMessage);
            final String hisShortUrl = historyMessage.getHttpService().toString();
            if (hisShortUrl.equals(shortUrl)) {
                final List<String> hisHeaders = hisAnalyzedRequest.getHeaders();
                for (final String hisHeader : hisHeaders) {
